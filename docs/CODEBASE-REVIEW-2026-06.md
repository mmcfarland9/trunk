...
| **PM-2** | Decide & document the derivation-architecture stance — ✅ **done 2026-06-03**: chose (B) keep divergence + lock with tests; written into `ARCHITECTURE.md` | Parity | **P2** | S (decision) |
...
**Progress (2026-06-03):**
- ✅ **Radar scores** + **watering streak** added to `derivation-parity.json` `expectedState`, with assertions in **both** `web/src/tests/parity.test.ts` (verified — full web suite green, 1414 tests) and `ios/TrunkTests/ParityTests.swift` (added; runs on iOS CI). **Timezone gotcha (caught by CI, PR #3):** the streak buckets days by the **6am-local** reset, so its value is timezone-sensitive. The fixture is authored for **UTC** and `vitest.config.ts` now pins `TZ=UTC` so local runs match CI (both CI runners are UTC). Radar scores are date-independent and unaffected. The radar weight constants and harvest formula were checked by hand and are **identical** across platforms (`W_WATER=0.05`, `W_SUN=0.35`, ceiling `100`, harvest `= soilCost × resultMultiplier`), so these should pass on iOS — the new test simply *guarantees* it going forward.
...
- **Resolved 2026-06-03:** PM-2 decided in favor of **(B)** — the soil-history divergence is now *documented as intentional* in `ARCHITECTURE.md`, with the rule "assert the **bucketed** output, not the raw series." So the remaining PM-1 work is to add a *bucketed* soil-history assertion (deferred — bucketing has its own range/calendar logic worth a dedicated fixture). Note: `cross-platform-validation.json` turned out to be a **formula-unit** fixture (soilCost/capacityGained/resetTime), web-only — not an event-replay fixture — so radar/streak correctly live in `derivation-parity.json` (consumed by both platforms) and don't belong there.

### PM-2 — Pick a stance on the architectural divergence — ✅ decided (B) *(P2, decision only)*

**Resolved 2026-06-03: chose (B).** A new "Cross-Platform Derivation Parity" section in `ARCHITECTURE.md` documents the structural divergence, *why* it's intentional, that fixtures are the safety net, and the soil-history "assert the bucketed output" rule. Original options, for the record:
